...
class TelloDetect():
    def __init__(self):
        ...
        self.hsv_dict={
            'yellow':[_r(0.120,0.4467,0.3333),_r(0.1998,1.0,1.0)],
            'black':[_r(0,0,0),_r(1,1,0.12)],  #黑的下限很低了？
    ...
    def detect(self, img, x=0, y=0, w=0, h=0,width=960,height=720,show=False,min_radius_k=10,max_radius_k=3):
        ...
        if show==True  :
            ...
            image=cv2.add(cv_image, np.zeros(np.shape(cv_image), dtype=np.uint8), mask=f_mask_2*255)
...
if __name__ == '__main__':
    ...
    for i, line in enumerate(lines):
        ...
        print('------start to det----------')
        t1=time.time()
        res=td.detect(img,width=img.shape[0],height=img.shape[1],show=True)
        dis_t=time.time()-t1
